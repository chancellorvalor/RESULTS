(() => {
  ...
  const els = {
    ...
    saveCandidates: document.getElementById("save-candidates"),
    candidateFields: document.getElementById("candidate-fields"),
  ...
  async function init() {
    ...
    els.saveCandidates.onclick = saveCandidates;
  ...
  async function loadData() {
    try {
      ...
      if (stateRows.length === 0) {
        showError("Election loaded, but no state rows were found. Run the Supabase seed query again or check state_results count.");
      }
    } catch (err) {
      showError("Could not load admin data. " + (err.message || err));
    }
  }
  ...
  function getCandidates() {
    try {
      const parsed = Array.isArray(election.candidates)
        ? election.candidates
        : JSON.parse(election.candidates || "[]");
  ...
  function renderCandidates() {
    ...
    els.candidateFields.innerHTML = candidates.map((c, i) => `
      <div class="candidate-editor" data-candidate-index="${i}">
        <label>
          Candidate ID
          <input data-candidate-field="id" value="${esc(c.id || "")}" readonly>
        </label>

        <label>
          Party Label
          <input data-candidate-field="party" value="${esc(c.party || "")}">
        </label>

        <label>
          Full Name
          <input data-candidate-field="name" value="${esc(c.name || "")}">
        </label>

        <label>
          Short Name
          <input data-candidate-field="shortName" value="${esc(c.shortName || "")}">
        </label>

        <label>
          Color
          <input data-candidate-field="color" value="${esc(c.color || "")}">
        </label>

        <label class="wide">
          Photo URL
          <input data-candidate-field="image" value="${esc(c.image || "")}" placeholder="https://example.com/photo.jpg">
        </label>
      </div>
    `).join("");
  }
  ...
  async function saveCandidates() {
    try {
      ...
      const candidates = [...els.candidateFields.querySelectorAll(".candidate-editor")].map(box => {
        const field = name => box.querySelector(`[data-candidate-field="${name}"]`)?.value?.trim() || "";
      ...
      showError("Could not save candidates. " + (err.message || err));
    }
  }
  ...
  function renderRows() {
    ...
    const filteredRows = stateRows.filter(r =>
      !q ||
      r.state_name.toLowerCase().includes(q) ||
      r.abbr.toLowerCase().includes(q)
    );

    els.rows.innerHTML = filteredRows.map(r => `
      <tr data-id="${r.id}">
        <td>${esc(r.state_name)} <small>${esc(r.abbr)}</small></td>

        <td>
          <input data-field="electoral_votes" type="number" value="${num(r.electoral_votes)}">
        </td>

        <td>
          <input data-field="total_turnout" type="number" value="${num(r.total_turnout)}">
        </td>

        <td>
          <input data-field="turnout_pct" type="number" step="0.1" min="0" max="100" value="${num(r.turnout_pct)}">
        </td>

        <td>
          <input data-field="gop_pct" type="number" step="0.1" min="0" max="100" value="${num(r.gop_pct)}">
        </td>

        <td>
          <input data-field="dem_pct" type="number" step="0.1" min="0" max="100" value="${num(r.dem_pct)}">
        </td>

        <td>
          <input data-field="ind_pct" type="number" step="0.1" min="0" max="100" value="${num(r.ind_pct)}">
        </td>
        ...
        <td>
          <button class="save-state" data-save="${r.id}">Save</button>
        </td>
      </tr>
    `).join("");
